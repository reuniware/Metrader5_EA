   ...
   if(done == false)
     {
      ...
      if (previous_ask>0 && ask!=previous_ask) {
         ...
         if (show_value_after == true) {
            if (show_pump_and_dump) printf("THEN = " + evol);
            show_value_after = false;
         }
         if (evol >= pump_dump_trigger || evol < -pump_dump_trigger) {
            if (evol > 0) {
               if (show_pump_and_dump) printf("PUMP = " + evol);
               if (evol > greatest_pump) {
                  greatest_pump = evol;
                  printf("New Greatest Pump = " + greatest_pump);
               }
            } else if (evol < 0) {
               if (show_pump_and_dump) printf("DUMP = " + evol);
               if (evol < greatest_dump) {
                  greatest_dump = evol;
                  printf("New Greatest Dump = " + greatest_dump);
               }
            }

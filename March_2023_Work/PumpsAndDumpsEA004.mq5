...
double pump_dump_trigger = 3; // 15 usd between 2 ticks is a strong move on BTCUSD on 21 03 2023 in the evening around 10pm UTC+2
            ...
            if(previous_bid >= mql_rates[1].low && bid <= mql_rates[1].low)
              {
               ...
               Comment("current price is getting below previous candlestick's low");
              }
            if(previous_bid <= mql_rates[1].high && bid >= mql_rates[1].high)
              {
               PlaySound("alert2.wav");
               printf("current price is getting above previous candlestick's high");
               Comment("current price is getting above previous candlestick's high");
              }
           }

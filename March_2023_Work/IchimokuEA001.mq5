...
//|                                             https://www.mql5.com |
//| EURUSD 01 01 2023 to 19 03 2023 M15 (nb lots = 0.1)              |
//| Capital 10000 Total profit 289.32                                |
//| Broker AdmiralMarkets-Demo                                       |
//+------------------------------------------------------------------+
      ...
      if(handle != INVALID_HANDLE)
        {
         ...
         if(nbk > 0)
           {
            //ArrayPrint(chikou_span_buffer);
            //printf(chikou_span_buffer[26]);
            //ArrayPrint(senkou_span_a_buffer);
            //printf(senkou_span_a_buffer[26]);
            //printf(senkou_span_b_buffer[26]);
            ...
            if(isNewBar())
              {
               ...
               if(senkou_span_b_buffer[1] > senkou_span_a_buffer[1])
                  if(mql_rates[1].open < senkou_span_b_buffer[1])
                     if(mql_rates[1].close > senkou_span_b_buffer[1])
                        if(mql_rates[1].close > tenkan_sen_buffer[1])
                           if(mql_rates[1].close > kijun_sen_buffer[1])
                              if(bid>senkou_span_b_buffer[0] && bid>kijun_sen_buffer[0] && bid>tenkan_sen_buffer[0])

                                {
                                 if(chikou_span_buffer[26] > senkou_span_a_buffer[26])
                                    if(chikou_span_buffer[26] > senkou_span_b_buffer[26])
                                       if(chikou_span_buffer[26] > tenkan_sen_buffer[26])
                                          if(chikou_span_buffer[26] > kijun_sen_buffer[26])
                                             if(chikou_span_buffer[26] > mql_rates[26].high)
                                                if(PositionsTotal()==0)
                                                   Trade_buy_2();
                                 if(showIchimokuInfos)
...
double tp = 1.5; // take profit in %
double sl = 0.25; // stop loss in %
